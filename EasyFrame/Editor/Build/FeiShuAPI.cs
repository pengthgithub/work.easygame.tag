...
public class FeiShuAPI 
{
   ...
   public static void Init()
   {
    ...
    private static async Task<string> get_access_token()
    {
        ...
        using (UnityWebRequest www = UnityWebRequest.Post(BuildScript.config.accessUrl, json))
        {
    ...
    private static async Task<bool> send_group_message(string imgKey)
    {
        ...
        var json = "{\"msg_type\":\"text\",\"content\":{\"text\":\" "+ Application.productName +" 预览版二维码,时效25分钟,耗时:"+useTime+"分\"}}";
    ...
    public static async void SendMessage(string content,int type = 0)
    {
        if (type == 0)
        {
            var json = "{\"msg_type\":\"text\",\"content\":{\"text\":\" "+ content +"分\"}}";
            var result = await send_message(json);
        }else if (type == 1)
        {
            var json = "{\"msg_type\":\"image\",\"content\":{\"image_key\":\""+content+"\"}}";
            var result = await send_message(json);
        }
    }
