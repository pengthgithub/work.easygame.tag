...
public class CameraManager : MonoBehaviour
{
    ...
    [SerializeField] [Rename("摄象机位置(调试)")] private bool _camaerAni = false;
    ...
    public void Awake()
    {
        ...
        if (wujianRoot == null)
        {
            wujianRoot = gameObject.AddComponent<SceneRoot>();
        }
    ...
    public void ApplyScreenWidth()
    {
        ...
        float displayAspect = uiAspect;
        if (aspect.Equals(displayAspect))
        {
            return;
        }
    ...
    public bool CameraAnimation
    {
        ...
        set
        {
            ...
            if (_cameraType == 0 || _cameraType == 1)
            {
                _animation.Play(singleCameraClip.name);
            }

            if (_cameraType == 2)
            {
                _animation.Play(mutlCameraClip.name);
            }
        }
    }
#if UNITY_EDITOR
    ...
    private void OnDrawGizmos()
    {
        ...
        var end = this.transform.forward * dis;
    ...
    private float _aniTotalTime = 0;
    private float _aniBackTime = 0;
    ...
    public void CameraShark(float dir, float power)
    {
        ...
        if (!IsPlayAnimation())
        {
            _defaultPoision = MainCamera.transform.position;
        }
    ...
    private void Update()
    {
        if (_aniTime > 0)
        {
            ...
            {
                Vector3 tempPos;
                float time = _aniTotalTime - _aniTime;
        ...
        else if (_aniBackTime > 0)
        {
            ...
            float num = _defaultPoision.x - MainCamera.transform.position.x;
            float num2 = _defaultPoision.y - MainCamera.transform.position.y;
