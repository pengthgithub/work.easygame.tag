...
namespace Easy
{
    ...
    public abstract class ELoader
    {
        ...
        public static void DestoryAsset(string url)
        {
            if (string.IsNullOrEmpty(url)) return;
            var rs = OriginSource.TryGetValue(url, out ObjectReference result);
            if (rs && result.OriginObject != null)
            {
                Addressables.Release(result.OriginObject);
            }
