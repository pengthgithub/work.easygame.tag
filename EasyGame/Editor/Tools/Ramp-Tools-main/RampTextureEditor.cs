...
public class RampTextureEditor
{
    ...
    public void Init(Material material)
    {
        ...
        {
            string materialPath = AssetDatabase.GetAssetPath(_rampTextureData);

            string path = AssetDatabase.GetAssetPath(material);
            path = path.Replace(".mat", ".asset");
            if (materialPath != path) _rampTextureData = null;
        }
    }
    ...
    public void InitScriptable(Material material, float val)
    {
        ...
        for (int i = _gradients.Count; i < 1; i++)
        {
